...
class MCSimulation:
    ...
    def __init__(self, portfolio_data, weights="", num_simulation=1000, num_trading_days=252):
       ...
       # if not "daily_return" in
        #portfolio_data.columns.get_level_values(2).unique():
        close_df = portfolio_data.xs('close',level=1,axis=1).pct_change() #1 &1
        tickers = portfolio_data.columns.get_level_values(0).unique()#0
